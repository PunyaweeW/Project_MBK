...
app.post("/part", function (req, res) {
  ...
  pool.getConnection(function (err, connection) {
    ...
    connection.query('INSERT INTO part_stock SET ?', newPart, (error, results) => {
      if (error) {
...
app.get("/sales/:startDate/:endDate", function (req, res) {
  pool.getConnection(function (err, connection) {
    if (err) { console.log(err); }
    connection.query('SELECT * FROM part_stock RIGHT JOIN (SELECT  barcode , COUNT(actionId) AS duratSales FROM log WHERE actionId = 20 AND datetime BETWEEN ? AND ? GROUP BY barcode , actionId) AS sales using (barcode)', [req.params.startDate, req.params.endDate], (error, results) => {
      if (error) {
...
app.post("/logging", function (req, res) {
  ...
  pool.getConnection(function (err, connection) {
    if (err) { console.log(err); }
    connection.query('INSERT INTO log SET ?', [newLog], (error, results) => {
      if (error) {
        //response.send("cannot create new log, please check specified barcode")
        res.send(error)
        connection.release();
...
app.delete("/logging", function (req, res) {
  pool.getConnection(function (err, connection) {
    if (err) { console.log(err); }
    connection.query('DELETE FROM log WHERE datetime BETWEEN ? AND ?', [req.body.startDate, req.body.endDate], (error, results) => {
      if (error) {
        //response.send("cannot create new log, please check specified barcode")
        res.send(error)
        connection.release();
