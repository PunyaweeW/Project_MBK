...
var Log = function(log){
  ...
  this.actionNumber = log.actionNumber;
};
//handling datetime+++++++++++++++++++++++++++++++++++++++++++++++++++++++
function NOW() {
...
app.get("/sales/:startDate/:endDate",function(req,res){
 pool.query('SELECT * FROM part_stock RIGHT JOIN (SELECT  barcode , SUM(actionNumber) AS  duratSales FROM log WHERE actionId = 20 AND datetime BETWEEN ? AND ? GROUP BY barcode , actionId ORDER BY duratReport DESC ) AS sales using (barcode)',[req.params.startDate,req.params.endDate], (error, results) => {
    if (error) {
